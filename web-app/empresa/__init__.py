...
# raw → enriched → classified → notifications chain. Admin-only (@admin_required)
# and scoped by business_id, so the business admin can discard any member's
# transaction within their business — and only within it.
...
@empresa_bp.route("/transacciones/editar")
@admin_required
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (enriched_id, session["business_id"]),
            )
            updated = cur.rowcount
        conn.commit()
        if updated:
