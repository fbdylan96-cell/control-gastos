...
def _upsert_categories(pairs, individual_id):
    """Insert categories scoped to a specific individual under the sentinel business."""
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            for category, subcategory in pairs:
                cur.execute(
                    ...
                    """,
                    (
                        str(uuid.uuid4()), INDIVIDUAL_BIZ_ID, individual_id, category, subcategory,
...
def categorias():
    if request.method == "POST":
        ...
        if action == "manual":
            ...
            if pairs:
                _upsert_categories(pairs, session["user_id"])
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                """
                SELECT id, category, subcategory, individual_id
                FROM core.categories
                WHERE business_id = %s
                  AND (individual_id = %s OR individual_id IS NULL)
                ORDER BY category, subcategory NULLS FIRST
